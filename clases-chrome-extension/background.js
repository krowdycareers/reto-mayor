...
//     chrome.tabs.sendMessage(tabs[0].id, { html: generateJobsHTML() });
//   });
// };

// const generateJobsHTML = () => {
//   let jobsHtml = "";
//   iteratorJobsInit.map((job) => {
//     jobsHtml += `
//       <div class="job">
//         <h2>${job.title}</h2>
//         <p>${job.description}</p>
...
// const saveJobsToLocalStorage = async (jobs) => {
  // const storedJobs = await getJobsFromLocalStorage();

  // const allJobs = [...storedJobs, ...jobs];

  // iteratorJobsInit = [...allJobs];

  // generateJobsHTML();
  // imprimirHTML()

  // chrome.storage.local.set({ jobs: allJobs }, () => {
    // console.log("Los trabajos se han guardado en el almacenamiento local");
  // });
// };
...
// getJobsFromLocalStorage().then((storedJobs) => {
//   iteratorJobsInit = [...storedJobs];

//   if (iteratorJobsInit.length > 0) {
//     generateJobsHTML();
//     imprimirHTML()
//   }
// });
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  port.onMessage.addListener(async (params, sender) => {
    ...
    if (cmd == "getInfo") {
      ...
      // saveJobsToLocalStorage(jobsInformation);
      jobs = [...jobs, ...jobsInformation];

      if (nextPage && counter < 8) {
        const {
          sender: {
            tab: { url, id },
          },
