...
chrome.runtime.onConnect.addListener(function (port) {
  port.onMessage.addListener(async function (params, sender) {
    ...
    if (cmd === "getInfo") {
      ...
      if (nextPage) {
        const { sender: { tab: { url, id: tabid } } } = sender
        ...
        function obtenerCiudadesTexto(ciudad) {
          ...
          if(ciudad==""){
            ciudad="Ciudad no mostrada"
          }
          return ciudad;
        }
        
        const JobsCitySalaryGroups = jobsInformation.reduce((cityGroups, job) => {

          const ciudad = obtenerCiudadesTexto(job.ciudad);
          const salario = job.salario.replace("Mensual", "").replace("\n+ comisiones", "").replace("Por Hora","");
        ...
        const jobCountByCitySalary = Object.keys(JobsCitySalaryGroups).reduce((acc, city) => {
          acc[city] = Object.keys(JobsCitySalaryGroups[city]).reduce((acc2, salary) => {
